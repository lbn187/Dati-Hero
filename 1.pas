...
begin
  ...
  writeln('------------------------------------------------------------------------------');
  repeat
  mm:=mm-1;delay(10);gotoxy(1,1);textcolor(sjys);write('剩余时间：');if mm div 100<10 then write('0');write(mm div 100,':');if mm mod 100<10 then write(0);write(mm mod 100);textcolor(qtzt);
  keypressed;
  ...
  f3:=true;dec(e);dec(shuaxin);sss:='\\st23021\dati\thing/'+number;
...
begin
 ...
 repeat ppp;until f3=false;
  ...
  if ee-1>=1 then begin assign(qt1,'\\st23021\dati\record/count.txt');reset(qt1);readln(qt1,ai);close(qt1);
  assign(qt2,'\\st23021\dati\record/count.txt');rewrite(qt2);writeln(qt2,ai+1);close(qt2);
  str(ai+1,sss1);sss1:='\\st23021\dati\record/'+sss1;sss:=name;iiiiii:=name2;
...
begin
 ...
 repeat
  ...
  x:=random(284)+1;;str(x,s1);s1:='\\st23021\dati\data/'+s1+'.txt';assign(pp,s1);reset(pp);
  for w:=0 to 5 do
...
exp:=exp-5;gotoxy(1,12+ans);writeln('金币-5，经验-5，时间-7秒');mm:=mm-700;end;
  ...
  writeln('按任意键继续');
  assign(qt3,'\\st23021\dati\tzms/count.txt');reset(qt3);readln(qt3,ai);close(qt3);
  assign(qt3,'\\st23021\dati\tzms/count.txt');rewrite(qt3);writeln(qt3,ai+1);close(qt3);
  str(ai+1,sss1);sss1:='\\st23021\dati\tzms/'+sss1;sss:=name;iiiiii:=name2;
...
writeln('-----------------------------------问题求解-----------------------------------');
  ...
  sss:='\\st23021\dati\wtqjdt\'+number;
...
begin
  repeat
  clrscr;
  x:=random(62)+1;str(x,ss);ss:='\\st23021\dati\ydcx\'+ss+'.txt';
  ...
  sss:='\\st23021\dati\ydcxdt\'+number;assign(bj,sss);reset(bj);readln(bj,tot);close(bj);
...
begin
  ...
  for i:=1 to 20 do
    begin
      str(tm[i],ss);ss:='\\st23021\dati\data/'+ss+'.txt';assign(bt,ss);reset(bt);
  ...
  repeat
    ...
    str(tm[zhizhen],ss);ss:='\\st23021\dati\wtqj/'+ss+'.txt';
  ...
  for i:=21 to 22 do
    begin
      ...
      str(tm[i],ss);ss:='\\st23021\dati\wtqj/'+ss+'.txt';
      assign(bt,ss);reset(bt);
      for ii:=1 to 50 do
...
begin
 repeat//1
  clrscr;ss1:='\\st23021\dati\sjjwj\'+number;
  ...
  str(x,s1);s1:='\\st23021\dati\time2/'+s1;jiaohuan;
  assign(pp,s1);reset(pp);readln(pp,rans);readln(pp,wans);readln(pp,tans);close(pp);
...
begin
  ...
  ss:='\\st23021\dati\thing/'+number;assign(bj,ss);reset(bj);for i:=1 to 7 do readln(bj,r[i]);close(bj);
...
begin
  ...
  for i:=4 to 13 do writeln(bj,s[i]);close(bj);ss:='\\st23021\dati\thing/'+number;assign(bj,ss);reset(bj);
...
begin
  ...
  for i:=5 to 13 do writeln(bj,s[i]);close(bj);ss:='\\st23021\dati\thing/'+number;assign(bj,ss);reset(bj);
  for i:=1 to 7 do readln(bj,r[i]);close(bj);assign(bj,ss);rewrite(bj);
...
begin
  ...
  writeln(bj,exp);writeln(bj,s[13]);close(bj);ss:='\\st23021\dati\thing/'+number;assign(bj,ss);reset(bj);
  for i:=1 to 7 do readln(bj,r[i]);close(bj);assign(bj,ss);rewrite(bj);
...
begin
  if a[5]>0 then begin
  f:=3;sss:='\\st23021\dati\achi/'+number;
...
begin
  f:=0;e[0]:=0;
  repeat
  clrscr;assign(bj,'\\st23021\dati\商店.txt');reset(bj);
  ...
  ss:='\\st23021\dati\thing/'+number;assign(bj,ss);reset(bj);
...
begin
  clrscr;
  assign(qt2,'\\st23021\dati\record/count.txt');reset(qt2);readln(qt2,ai);close(qt2);
  for i:=1 to ai do
    begin str(i,s);s:='\\st23021\dati\record/'+s;
...
begin
  clrscr;
  assign(qt2,'\\st23021\dati\asus/count.txt');reset(qt2);readln(qt2,ai);close(qt2);
  for i:=1 to ai do
...
begin
  clrscr;
  assign(qt2,'\\st23021\dati\tzms/count.txt');reset(qt2);readln(qt2,ai);close(qt2);
  for i:=1 to ai do
...
begin
  clrscr;
  assign(qt2,'\\st23021\dati\tieba/count.txt');reset(qt2);readln(qt2,ai);close(qt2);
  for i:=1 to ai do
...
begin
  ...
  assign(qt2,'\\st23021\dati\tieba/count.txt');reset(qt2);readln(qt2,ai);close(qt2);
  for i:=1 to ai do
    begin str(i,s);s:='\\st23021\dati\tbll/'+s;
    assign(qt2,s);reset(qt2);readln(qt2,aa[i]);close(qt2);
    str(i,s);s:='\\st23021\dati\tbfz/'+s;
    assign(qt2,s);reset(qt2);readln(qt2,ss1[i]);close(qt2);
    str(i,s);s:='\\st23021\dati\tieba/'+s+'.txt';
    assign(qt2,s);reset(qt2);readln(qt2,iiiii[i]);close(qt2);end;
  for i:=ai downto 1 do
    for j:=1 to i-1 do
      if aa[j]<aa[j+1] then begin
...
begin
  repeat
  clrscr;assign(bt,'\\st23021\dati\英雄榜.txt');reset(bt);
...
begin
  ...
  assign(bj,'\\st23021\dati\tieba/count.txt');reset(bj);readln(bj,ans);close(bj);
  assign(bj,'\\st23021\dati\tieba/count.txt');rewrite(bj);writeln(bj,ans+1);close(bj);
  str(ans+1,ss);ss:='\\st23021\dati\tieba/'+ss+'.txt';assign(bj,ss);rewrite(bj);
  ...
  str(ans+1,ss);ss:='\\st23021\dati\tbfz/'+ss;assign(bj,ss);rewrite(bj);
...
begin
  str(bianhao,sss);sss:='\\st23021\dati\tbfz/'+sss;assign(bj,sss);reset(bj);
  ...
  assign(bj,mynumber);rewrite(bj);for ii:=1 to 13 do writeln(bj,s[ii]);close(bj);
  str(bianhao,ss);ss:='\\st23021\dati\tieba/'+ss+'.txt';
  assign(bj,'\\st23021\dati\asus/count.txt');reset(bj);readln(bj,cou);close(bj);
  assign(bj,ss);reset(bj);
  ...
  for j:=1 to i-1 do writeln(bj,sr[j]);close(bj);
  str(bianhao,sss);sss:='\\st23021\dati\tbhf/'+sss;
...
begin
  ...
  assign(bj,'\\st23021\dati\tieba/count.txt');reset(bj);readln(bj,ans);close(bj);
  for i:=1 to ans do
    begin str(i,sss);sss:='\\st23021\dati\tieba/'+sss+'.txt';
          assign(bj,sss);reset(bj);readln(bj,s[i]);close(bj);end;
  for i:=1 to ans do
    begin
      str(i,sss);sss:='\\st23021\dati\tbhf/'+sss;assign(bj,sss);reset(bj);readln(bj,hf[i]);close(bj);
      str(i,sss);sss:='\\st23021\dati\tbll/'+sss;assign(bj,sss);reset(bj);readln(bj,ll[i]);close(bj);
    end;
  yeshu:=ans div 10+1;if ans mod 10=0 then dec(yeshu);zhizhen:=1;zhizhen2:=1;
  repeat
  clrscr;
  assign(bj,'\\st23021\dati\贴吧.txt');reset(bj);
  ...
  until ord(c)=13;
  bianhao:=zhizhen*10+zhizhen2-10;str(bianhao,ss);ss:='\\st23021\dati\tieba/'+ss+'.txt';
  assign(bj,ss);reset(bj);
  for i:=1 to 10000 do begin readln(bj,s[i]);if s[i]='' then break;end;close(bj);
  str(bianhao,ss);ss:='\\st23021\dati\tbll/'+ss;
...
begin
  ...
  assign(qt,'\\st23021\dati\computer/首页背景.txt');reset(qt);
  readln(qt,op);textbackground(op);close(qt);
  assign(qt,'\\st23021\dati\computer/首页字体颜色.txt');reset(qt);readln(qt,op);
  textcolor(op);close(qt);
  assign(qt,'\\st23021\dati\computer/错误选项背景.txt');reset(qt);readln(qt,cuowu);close(qt);
  assign(qt,'\\st23021\dati\computer/答题时间.txt');reset(qt);
  readln(qt,dati);dati:=dati*100;close(qt);
  assign(qt,'\\st23021\dati\computer/时间颜色.txt');
  reset(qt);readln(qt,sjys);close(qt);
  assign(qt,'\\st23021\dati\computer/双人模式答错机会.txt');
  reset(qt);readln(qt,dcjh);close(qt);
  assign(qt,'\\st23021\dati\computer/题目颜色.txt');
  reset(qt);readln(qt,tmys);close(qt);
  assign(qt,'\\st23021\dati\computer/题目背景.txt');
  reset(qt);readln(qt,tmbj);close(qt);
  assign(qt,'\\st23021\dati\computer/选项颜色.txt');
  reset(qt);readln(qt,xuanxiang);close(qt);
  assign(qt,'\\st23021\dati\computer/正确选项背景.txt');
  reset(qt);readln(qt,zhengque);close(qt);
  assign(qt,'\\st23021\dati\computer/停留时间.txt');
  reset(qt);readln(qt,tlsj);close(qt);
  assign(qt,'\\st23021\dati\computer/其他字体颜色.txt');
  reset(qt);readln(qt,qtzt);close(qt);
  assign(bj,'\\st23021\dati\背景.txt' );reset(bj);
  for i:=1 to 24 do begin readln(bj,ss);writeln(ss);end;close(bj);
  sssyy:='\\st23021\dati\thing/'+number;assign(bj,sssyy);reset(bj);
...
begin
  clrscr;assign(bj,'\\st23021\dati\注册.txt');reset(bj);
  for i:=1 to 24 do begin readln(bj,ss);writeln(ss);end;close(bj);
  ...
  assign(bj,'\\st23021\dati\asus/count.txt');reset(bj);readln(bj,ans);close(bj);
  for i:=1 to ans do
    begin
      str(i,sss);sss:='\\st23021\dati\asus/'+sss;assign(bj,sss);reset(bj);readln(bj,ss);close(bj);
      if ss=s[1] then begin fff:=false;write('已有此用户，按任意键继续');end;end;end;
  if fff=true then begin
  assign(bj,'\\st23021\dati\asus/count.txt');rewrite(bj);writeln(bj,ans+1);close(bj);str(ans+1,sss);sss:='asus/'+sss;
  assign(bj,sss);rewrite(bj);
  for i:=1 to 10 do writeln(bj,s[i]);
  writeln(bj,0);writeln(bj,0);writeln(bj,1);writeln(bj,0);close(bj);
  str(ans+1,sss);sss:='\\st23021\dati\achi/'+sss;assign(bj,sss);rewrite(bj);writeln(bj,'0 0 0');close(bj);
  str(ans+1,sss);sss:='\\st23021\dati\thing/'+sss;
...
begin
  ...
  repeat
  clrscr;assign(bj,'\\st23021\dati\首页.txt');reset(bj);
  ...
  assign(bj,'\\st23021\dati\asus/count.txt');reset(bj);readln(bj,ans);close(bj);f1:=false;f2:=false;
  for i:=1 to ans do
  begin
    str(i,sss);sss:='\\st23021\dati\asus/'+sss;assign(bj,sss);reset(bj);readln(bj,ss3);readln(bj,ss4);
  ...
  ttt:='\\st23021\dati\asus/'+ttt;assign(bj,ttt);reset(bj);
  ...
  ttt2:='\\st23021\dati\achi/'+ttt2;assign(bj,ttt2);reset(bj);readln(bj,rig,wro,rec);close(bj);go;
  end; end else zc;
